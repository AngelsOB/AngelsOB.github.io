...
import { singleHopIBU } from '@/calculators/ibu';
...
export class RecipeCalculationService {
  ...
  /**
   * Calculate IBU contribution from a single hop addition. Delegates to the
   * shared `singleHopIBU` (see @/calculators/ibu) so the Tinseth/humulinone
  ...
  calculateSingleHopIBU(
    ...
    return singleHopIBU(hop, og, ibuVolumeGal, boilGravity, boilTimeMin);
  }
