...
export default function GrainTweaker({
  ...
  params: GrainParams;
  onChange: (p: GrainParams) => void;
}) {
  const [isOpen, setIsOpen] = useState(false);

  const set = (key: keyof GrainParams, value: number) =>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-2">
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <span
              ...
              style={{ color: "var(--brew-accent-600)", letterSpacing: "0.18em" }}
            >
              Grain
            </span>
            <button
              onClick={() => onChange(GRAIN_DEFAULTS)}
              className="text-xs"
          ...
          <div className="space-y-3">
            <Slider label="Opacity" value={params.opacity} min={0} max={0.8} step={0.01} onChange={(v) => set("opacity", v)} />
            ...
            <Slider label="Soft-light mix" value={params.softLight} min={0} max={1} step={0.05} onChange={(v) => set("softLight", v)} />
      ...
      <button
        ...
        className="brew-btn-ghost !rounded-xl !px-3 !py-2 text-xs font-semibold shadow-lg"
      >
        ◈ Grain
      </button>
    </div>
