...
export async function POST(req: NextRequest) {
  ...
  let slug = generateShareSlug(recipeData.name as string);
  ...
  if (!existingSlug.empty) {
    // Collision — regenerate with a different suffix
  ...
  // Write to publicRecipeIndex
  const hopNames = Array.isArray(recipe.hops)
    ? [...new Set(recipe.hops.map((h) => h.name))]
    : [];

  await adminDb.collection("publicRecipeIndex").doc(recipeId).set({
    name: recipe.name,
    style: recipe.style || "",
    ownerName,
    ownerId: userId,
