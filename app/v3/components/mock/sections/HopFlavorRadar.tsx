...
// Mini and full hop flavor radar SVG. Used both inline in the hop bill
// table rows AND as the sub-bone that explodes out on stage 4.
...
export function HopFlavorRadar({ size, variant, hopName }: Props) {
  ...
  const polygon = values
    .map((v, i) => point(i, v).map((n) => n.toFixed(2)).join(","))
    .join(" ");
    ...
    >
      ...
      {gridRings.map((s, i) => {
        const poly = Array.from({ length: n }, (_, ai) =>
          point(ai, s).map((n) => n.toFixed(2)).join(","),
        ).join(" ");
        return (
          <polygon
            key={`ring-${i}`}
      ...
      {variant === "full" &&
        AXES.map((label, i) => {
          const [lx, ly] = point(i, 1).map((n, j) =>
            j === 0
              ? n + (Math.cos(angle(i)) > 0 ? 4 : -4)
              : n + (Math.sin(angle(i)) > 0 ? 4 : -2),
          );
          const a = angle(i);
