...
export default function StageHops({
  onEnter,
  onLeave,
}: {
  onEnter?: () => void;
  ...
  // Each paragraph starts shortly after the previous one finishes revealing,
  // so the flow feels continuous without overlapping into noise.
  const paragraphDelays = useMemo(() => {
    let cursor = endDelay(STAGES.hops.lead, LEAD_DELAY) + GAP;
    return STAGES.hops.paragraphs.map((p) => {
      const start = cursor;
  ...
  return (
    ...
    >
      ...
      >
        <StageEyebrow>{STAGES.hops.h2}</StageEyebrow>
      </motion.div>
      ...
      >
        <ParagraphReveal
          text={STAGES.hops.lead}
          inView={inView}
          baseDelay={LEAD_DELAY}
        />
      </h3>

      {STAGES.hops.paragraphs.map((p, i) => (
        <p
          key={i}
          style={{
