...
export default function SectionHero({ recipeCount }: Props) {
  ...
  const isSignedIn = !!user && recipesLoaded;
  const isSignedInWithRecipes = isSignedIn && recipes.length > 0;
  const isSignedInEmpty = isSignedIn && recipes.length === 0;
  ...
  return (
    ...
    >
      ...
      {isSignedInWithRecipes ? (
        ...
        <SignedInEmptyHero displayName={user?.displayName ?? null} />
      ) : (
...
// ─────────────────────────── Signed-in, empty library ───────────────────────────

/**
 * Signed in but no recipes yet. Welcome them with the canonical empty-library
 * line from voice-and-tone, plus a single Start CTA. Same right-column showcase
 * so they can see what the builder looks like before they make their first.
 */
function SignedInEmptyHero({ displayName }: { displayName: string | null }) {
  ...
  return (
    ...
    >
      <div style={{ minWidth: 0 }}>
        ...
        >
          <HSScriptNote color={hsTokens.yeast} size={26} rotate={-4}>
            {firstName ? `hello, ${firstName.toLowerCase()} —` : "hello, brewer —"}
          </HSScriptNote>
        </motion.div>
        ...
        >
          No recipes yet.
          <br />
          <span style={{ color: hsTokens.roast }}>Let&rsquo;s make one.</span>
        </motion.h1>
        ...
        >
          Pick a style, drop in grains and hops, and dial in your numbers. The
          math runs as you go.
        </motion.p>
        ...
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55, delay: 0.55, ease: SMOOTH }}
          style={{
            display: "flex",
            flexWrap: "wrap",
            gap: 14,
