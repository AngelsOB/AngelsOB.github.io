...
- **Yeast — DONE (uncommitted, 2026-06-07).** 143 → **524 strains**, **516 enriched (98%)** with type/form/temp/floc/attenuation(range+midpoint)/alcohol-tolerance/pof/sta1 + provenance. Method = **hybrid**: open BeerJSON facts (BrewTarget `DefaultContent003/004`, upstream brewerwall) for ~373, manufacturer-page scrape (curl + agents, facts-or-null, cite URL) for the gap (231 applied, 214 high-confidence). Cleaned out wine/bacteria/distilling/pitch-variants/dupes.
  ...
  - **UI consumers — DONE (2026-06-07, uncommitted):** Shared cursor-follow hover preview (`yeastHoverPreview.tsx`) — same panel surfaces from both the YeastPresetModal picker rows AND on hover anywhere over a pitched StrainCard, so the builder card stays compact (no permanent details row chrome). Preview body shows lab/name + type/form/temp/floc/atten/abv-tolerance stats + same-strain/subs lines. StrainCard now keeps only the ABV-tolerance flag as inline always-visible chip beneath the lab caption (honey for "approaching" within 1% of tolerance, roast for "exceeds"). Packaging auto-default (handleSelectPreset + handleApplyGenerator + both fallback defaults) derive yeastType from `form` (or category if absent — defensive, since all shipped presets have `form`). FermentationSection shows the strain's temp range in the eyebrow meta + a per-step "X°C over/under strain min/max" nudge on active-ferment steps (cold-crash/conditioning excluded). Shared formatters in `src/modules/hopskip/components/builder/yeastDetails.ts`. All paths tsc-clean + lint-clean; 8 yeast preset tests pass.
  - **Remaining:** add dmtaylor+suregork to in-product `/credits`; commit; (paid product) ask dmtaylor + IP-lawyer glance on EU DB-right.
...
- **Similar-HOPS UI — DONE (2026-06-07, uncommitted).** Cosine over the 9-axis flavor vector in `src/modules/hopskip/components/builder/hopDetails.ts` (`findSimilarHops`, top-N by similarity, source self + flavor-less library entries excluded). Surfaced via a new shared `useHopHoverPreview` hook (`src/modules/hopskip/components/builder/hopHoverPreview.tsx`) — same anchored-above + cursor-spring-follow + dwell-on-show pattern as `useYeastHoverPreview`, with the existing radar + similar-hops chips below. Wired into both the HopPresetModal (showDelay: 0, instant pop on row sweep) AND the builder HopSection rows (default 300ms dwell on the per-row mini radar). The section's old per-row cursor-follow portal + `hoveredRowFlavor` state were removed; `RowHoverPreviewBody` deleted (RowHoverMiniRadar kept — still used by the variety card rail). Saaz→noble hops, Cascade→Centennial/Ahtanum, Galaxy→Mosaic/Bru-1 spot-checked correct.
